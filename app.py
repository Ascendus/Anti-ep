...
class Edu:
    ...
    def __init__(self,email,password):
        ...
        self.bot =webdriver.Chrome("C:/Users/ascen/Downloads/chromedriver_win32/chromedriver.exe")
        print('Bot ready!')
    ...
    def login(self):
        ...
        while True: #Continue when loaded
            time.sleep(1)
            try:
                bot.find_element(By.ID, 'login-password')
                time.sleep(0.5)
                break
            except:
        ...
        print('Logging in!')
        print('         ')
        emailInp= bot.find_element(By.ID, 'login-username')
        passwrdInp= bot.find_element(By.ID, 'login-password')
        emailInp.clear()
        passwrdInp.clear()
    ...
    def lists(self):
        ...
        try:  #checks to see if it has multiple pages
           bot.find_element(By.ID, 'full-list-switcher').click()
           bot.find_element(By.ID, 'tutorial-hint-link').click()
           pages=True
           time.sleep(0.5)
        except:
            pages=False
        ...
        if pages == False:
            wordstring=bot.find_element(By.ID, 'word-count').text #find word count
            cutoff=wordstring.find('wo')
            amount=int(wordstring[0:cutoff])
            print(amount)
        else:
            pagenum=bot.find_element(By.ID, 'section-navigator-content').text
            loc=pagenum.find('f') 
    ...
    def listquestions(self):
        ...
        try:
            ...
            pass
        bot.find_element(By.ID, 'start-button-main').click() #start challenge
        time.sleep(1)
        finished=False
        #answers questions
        while finished == False:
        
            question=sanitiseInside(bot.find_element(By.ID, 'question-block').text)  #finds question
            try:
                ...
                try:
                    ...
                    bot.find_element(By.ID, 'submit-button').click()
                    bot.find_element(By.ID, 'submit-button').click() #goes ahead
                    correct=sanitiseInside(bot.find_element(By.ID, 'correct-answer-field').text)  #finds correct answer
            ...
            imp.send_keys(ans)
            submit=bot.find_element(By.ID, 'submit-button')
            try:
                submit.click()
            except:
                try:
                    correct=sanitiseInc(bot.find_element(By.ID, 'correct-answer-field').text) #corrects anwser
                ...
                bot.find_element(By.ID, 'continue-button').click()
                time.sleep(0.1)
            time.sleep(0.1)
            try:
